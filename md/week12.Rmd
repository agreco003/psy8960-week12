...
final_tbl <- right_join(topics_tbl, final_week12_tbl, by = "doc_id") %>% #retains all case numbers, even missing values. Check title vs original column to ensure it worked correctly
  ...
  rename(original = title) %>% #creating unordered factors for lm model later, renames titles column so all titles are present
  arrange(doc_id)
...
Unfortunately, it seems our categories are not predictive of how man upvotes a post receives. Though our intercept is significant, the model results in a very, very small r-squared value (~.005), suggesting our categories do not explain much of the variance in upvotes. 